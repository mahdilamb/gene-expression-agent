...
from agent.session import _key, load_messages, redis_ok, save_messages
...
async def test_load_messages_returns_stored_messages(fake_redis: Redis):
    ...
    await fake_redis.set(_key("s1"), json.dumps(messages))
...
async def test_load_messages_sessions_are_isolated(fake_redis: Redis):
    await fake_redis.set(_key("s1"), json.dumps([{"role": "user", "content": "one"}]))
    await fake_redis.set(_key("s2"), json.dumps([{"role": "user", "content": "two"}]))
...
async def test_save_messages_persists(fake_redis: Redis):
    ...
    await save_messages("s1", messages)

    raw = await fake_redis.get(_key("s1"))
...
def test_key_format():
    assert _key("abc-123") == "agent:session:abc-123", (
